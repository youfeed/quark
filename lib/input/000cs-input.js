  ...
  constructor({
    ...
    shadowRoot.innerHTML = `
        ...
        <cs-tips id="input-con" dir="${this.errordir}" type="error">
            ${this.icon ? '<cs-icon class="icon-pre" name=' + this.icon + '></cs-icon>' : ''}
            ...
            ${this.type === 'password' && !multi ? '<cs-button id="btn-pass" class="btn-right" icon="eye-close" type="flat" shape="circle"></cs-button>' : ''}
            ${this.type === 'search' && !multi ? '<cs-button id="btn-search" class="btn-right" icon="search" type="flat" shape="circle"></cs-button>' : ''}
            ${this.type === 'number' && !multi ? '<div class="btn-right btn-number"><cs-button id="btn-add" icon="up" type="flat"></cs-button><cs-button id="btn-sub" icon="down" type="flat"></cs-button></div>' : ''}
        </cs-tips>
        `;
...
class CsTextarea extends CsInput {
  constructor() {
    super({
      multi: true
    });
...
class CsInputGroup extends HTMLElement {
  constructor() {
    super();
    const shadowRoot = this.attachShadow({
      mode: 'open'
...
//# sourceMappingURL=000cs-input.js.map