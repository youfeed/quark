...
class CsTabContent extends HTMLElement {
...
export default class CsTab extends HTMLElement {
  static get observedAttributes() {
    return ['activekey'];
  }
  ...
  inittab() {
    const items = this.nav.querySelectorAll('.nav-item'); // console.log(items, 8877);
  ...
  connectedCallback() {
    ...
    this.slots.addEventListener('slotchange', () => {
      ...
      slots.forEach((item, index) => {
        if (item.tagName === 'CS-TAB-CONTENT') {
          ...
          html += `<cs-button class="nav-item ${item.key === this.activekey ? 'active' : ''}" icon=${item.icon} ${item.disabled !== null ? "disabled" : ""} data-key=${item.key}>${item.label}</cs-button>`;
        }
      });
    ...
    this.nav.addEventListener('click', ev => {
      const item = ev.target.closest('cs-button');
...
//# sourceMappingURL=cs-tab.js.map